...
def parseContent(bS, urlObj, content, evalJs, **kwargs):
    ...
    # 返回第一个nextUrl，让前端控制分页
    if nextContentUrls and len(nextContentUrls) >= 1:
        nextUrl = urljoin(urlObj['url'], nextContentUrls[0])
        # 检查nextUrl是否是下一章的URL
        if nextChapterUrl and nextUrl != nextChapterUrl:
            chapterContent['nextUrl'] = nextUrl
        elif not nextChapterUrl:
            # 如果没有提供nextChapterUrl，也返回nextUrl
            chapterContent['nextUrl'] = nextUrl

    # 注释掉自动循环获取的逻辑，改为由前端控制
    # if nextContentUrls:
