# Novel_v3 - 现代化小说阅读平台

基于 Legado 书源的全栈小说阅读网站，提供流畅的在线阅读体验和书籍管理功能。

## ✨ 核心功能

- 📚 **书源管理**：支持导入和管理 Legado 书源（JSON/URL）
- 🔍 **全局搜索**：跨所有书源搜索小说
- 📖 **在线阅读**：流畅的章节阅读体验
- 💾 **书库管理**：收藏喜欢的小说，支持下载到服务器
- 🎨 **现代界面**：响应式设计，支持深色/浅色主题
- 🚀 **高性能**：异步处理，快速响应

## 🛠️ 技术栈

### 前端
- **框架**：React 18 + TypeScript
- **构建工具**：Vite
- **样式**：Tailwind CSS
- **路由**：React Router v6
- **状态管理**：Zustand
- **HTTP 客户端**：Axios

### 后端
- **框架**：FastAPI (Python)
- **数据库**：SQLite + SQLAlchemy
- **书源解析**：LegadoParser2
- **异步支持**：aiosqlite

## 📦 项目结构

```
Novel_v3/
├── backend/                    # 后端服务
│   ├── app/
│   │   ├── api/               # API 路由
│   │   │   ├── book_sources.py    # 书源管理
│   │   │   ├── search.py          # 搜索功能
│   │   │   ├── books.py           # 书籍信息
│   │   │   ├── library.py         # 书库管理
│   │   │   └── explore.py         # 发现页面
│   │   ├── models/            # 数据模型
│   │   ├── services/          # 业务逻辑
│   │   ├── schemas/           # Pydantic 模型
│   │   ├── database.py        # 数据库配置
│   │   ├── config.py          # 应用配置
│   │   └── main.py            # 应用入口
│   ├── downloads/             # 下载的书籍
│   ├── novel.db               # SQLite 数据库
│   └── requirements.txt       # Python 依赖
│
├── frontend/                   # 前端应用
│   ├── src/
│   │   ├── pages/             # 页面组件
│   │   │   ├── Home.tsx           # 首页
│   │   │   ├── BookSources.tsx    # 书源管理
│   │   │   ├── BookDetail.tsx     # 书籍详情
│   │   │   ├── Library.tsx        # 我的书库
│   │   │   └── Reader.tsx         # 阅读器
│   │   ├── components/        # 公共组件
│   │   ├── services/          # API 服务
│   │   ├── stores/            # 状态管理
│   │   ├── types/             # TypeScript 类型
│   │   └── App.tsx            # 应用入口
│   ├── package.json
│   └── vite.config.ts
│
└── LegadoParser/              # 书源解析器（第三方）
```

## 🚀 快速开始

### 环境要求

- **Python**: 3.9+
- **Node.js**: 16+
- **npm**: 8+

### 本地开发

#### 1. 启动后端

```bash
# 进入后端目录
cd backend

# 安装依赖
pip install -r requirements.txt

# 启动服务（开发模式）
python -m uvicorn app.main:app --reload --host 0.0.0.0 --port 8000
```

后端服务运行在：`http://localhost:8000`
API 文档：`http://localhost:8000/docs`

#### 2. 启动前端

```bash
# 进入前端目录
cd frontend

# 安装依赖
npm install

# 启动开发服务器
npm run dev
```

前端应用运行在：`http://localhost:5173`

**注意**：开发环境下，Vite 会自动将 `/api` 请求代理到 `http://localhost:8000`（配置在 `vite.config.ts`）

### 生产部署

#### 方式一：使用 Nginx（推荐）

**1. 后端部署**

```bash
# 上传代码到服务器
scp -r backend root@your-server:/var/www/novel/

# SSH 连接到服务器
ssh root@your-server

# 安装依赖
cd /var/www/novel/backend
pip install -r requirements.txt

# 启动服务（生产模式）
nohup python -m uvicorn app.main:app --host 127.0.0.1 --port 8000 > backend.log 2>&1 &
```

**2. 前端部署**

```bash
# 在本地构建
cd frontend
npm run build

# 上传构建产物到服务器
scp -r dist root@your-server:/var/www/novel/frontend/
```

**3. 配置 Nginx**

创建 Nginx 配置文件 `/etc/nginx/sites-available/novel`：

```nginx
server {
    listen 80;
    server_name your-domain.com;  # 或使用 IP 地址

    # 前端静态文件
    location / {
        root /var/www/novel/frontend/dist;
        try_files $uri $uri/ /index.html;
    }

    # 后端 API 代理
    location /api/ {
        proxy_pass http://127.0.0.1:8000;
        proxy_http_version 1.1;
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection 'upgrade';
        proxy_set_header Host $host;
        proxy_cache_bypass $http_upgrade;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
    }
}
```

启用配置并重启 Nginx：

```bash
sudo ln -s /etc/nginx/sites-available/novel /etc/nginx/sites-enabled/
sudo nginx -t
sudo systemctl restart nginx
```

#### 方式二：直接运行（开发/测试）

```bash
# 后端
cd backend
python -m uvicorn app.main:app --host 0.0.0.0 --port 8000

# 前端（新终端）
cd frontend
npm run dev -- --host 0.0.0.0
```

访问：`http://your-server-ip:5173`


**访问地址**：
- 前端：`http://your-server-ip:5173`
- 后端 API：`http://your-server-ip:8000`

## 📖 使用指南

### 1. 添加书源

**方式一：JSON 文本**
1. 进入"书源管理"页面
2. 点击"添加书源"
3. 选择"JSON 文本"
4. 粘贴书源 JSON
5. 点击"添加"

**方式二：网络链接**
1. 进入"书源管理"页面
2. 点击"添加书源"
3. 选择"网络链接"
4. 输入书源 URL
5. 点击"添加"

**推荐书源**：笔趣阁（简单规则，兼容性好）

### 2. 搜索小说

- **首页搜索**：跨所有书源搜索
- **书源搜索**：在指定书源中搜索

### 3. 阅读和管理

1. 点击搜索结果查看书籍详情
2. 点击"收藏"添加到书库
3. 点击"下载到服务器"离线保存
4. 点击章节开始阅读

## ⚠️ 书源兼容性说明

### ✅ 支持的规则
- CSS 选择器：`.item`, `h3 a@text`
- XPath：`//div[@class='book']`
- JSONPath：`$.data.list[*]`
- 正则表达式

### ❌ 不支持的规则
- Legado APP 专有 API（`java.ajax()`, `java.base64Encode()` 等）
- 复杂的 JavaScript 代码

**推荐**：使用类似笔趣阁的简单规则书源。

## 🔧 API 接口

### 主要端点

| 端点 | 方法 | 说明 |
|------|------|------|
| `/api/v1/book-sources` | GET/POST | 书源管理 |
| `/api/v1/search` | GET | 全局搜索 |
| `/api/v1/books/{book_url}/info` | GET | 书籍信息 |
| `/api/v1/books/chapters` | GET | 章节列表 |
| `/api/v1/books/chapter/content` | GET | 章节内容 |
| `/api/v1/library` | GET/POST | 书库管理 |
| `/api/v1/library/{id}/download` | POST | 下载书籍 |

完整 API 文档：`http://localhost:8000/docs`

## 📝 开发说明

### 前端开发

```bash
cd frontend
npm run dev      # 开发服务器
npm run build    # 生产构建
npm run preview  # 预览构建结果
npm run lint     # 代码检查
```

### 后端开发

```bash
cd backend
python -m uvicorn app.main:app --reload  # 热重载模式
```

## 🤝 贡献

欢迎提交 Issue 和 Pull Request！

## 📄 许可证

本项目基于 LegadoParser 开发，遵循其开源协议。

